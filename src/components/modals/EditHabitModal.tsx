...
export const EditHabitModal = ({ isOpen, onClose, habitToEdit }: EditHabitModalProps) => {
  ...
  // Populate form data when modal opens or habitToEdit changes
  useEffect(() => {
    if (habitToEdit) {
      setFormData({
        name: habitToEdit.name || '',
        description: habitToEdit.description || '',
        category: habitToEdit.category || 'development',
        target_duration_minutes: habitToEdit.target_duration_minutes || 60,
        color: habitToEdit.color || '#4361EE'
      });
    }
  }, [habitToEdit]);
    ...
    <div className="fixed inset-0 bg-background/80 z-50 flex items-start justify-center overflow-y-auto p-4 pt-[10vh]">
      <div className="bg-surface-1 border border-border rounded-lg w-full max-w-md overflow-y-auto animate-fade-up mb-4" style={{ maxHeight: 'calc(100vh - 12vh)' }}>
        <div className="flex items-center justify-between p-5 border-b border-border">
          <h2 className="text-lg font-display font-semibold text-foreground">Edit Habit</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="name-edit" className="text-sm">Habit Name *</Label>
            <Input
              id="name-edit"
              value={formData.name}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="description-edit" className="text-sm">Description</Label>
            <Textarea
              id="description-edit"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="category-edit" className="text-sm">Category</Label>
            <Select
