...
import { OverallStreakHeatmap } from '@/components/dashboard/OverallStreakHeatmap';
...
const RoomDashboard = () => {
  ...
  const [memberTab, setMemberTab] = useState<'overview' | 'heatmap' | 'streak_heatmap'>('overview');
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-up">
      ...
      <div className="flex justify-center pt-2 pb-8">
        <Button variant="outline" onClick={() => setShowLeaveConfirm(true)} className="border-border hover:bg-destructive hover:text-destructive-foreground rounded-2xl px-6">
      ...
      <Dialog open={!!selectedMember} onOpenChange={() => setSelectedMember(null)}>
        <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto bg-surface-1 border-border rounded-2xl">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            ...
            <button onClick={() => setMemberTab('streak_heatmap')}
              className={`px-4 py-2 rounded-xl text-xs font-medium transition-colors ${
                memberTab === 'streak_heatmap' ? 'bg-orange-500 text-white' : 'bg-surface-2 text-muted-foreground'
              }`}>
              <Flame className="w-3.5 h-3.5 inline mr-1.5" />Streak Heatmap
            </button>
          </div>
          ...
          <div className="py-4 space-y-6">
            ...
            ) : memberTab === 'streak_heatmap' ? (
              /* Overall Streak Heatmap tab */
              <div className="space-y-4">
                {memberHabitsLoading ? (
                  <div className="text-center text-sm text-muted-foreground py-8">Loading streak heatmaps...</div>
                ) : memberHabits.length === 0 ? (
                  <div className="text-center text-sm text-muted-foreground py-8">No habits to show streaks for.</div>
                ) : (
                  <OverallStreakHeatmap habits={memberHabits} />
                )}
              </div>
            ) : null}
          </div>
      ...
      <AlertDialog open={!!habitToDelete} onOpenChange={() => setHabitToDelete(null)}>
        <AlertDialogContent className="bg-surface-1 border-border rounded-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display">Delete Shared Habit</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete "{habitToDelete?.name}"? This will remove all associated tracking data and cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="border-border hover:bg-surface-2 rounded-xl">Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => { if (habitToDelete) { deleteHabit.mutate(habitToDelete.id); setHabitToDelete(null); } }}
      ...
      <AlertDialog open={showLeaveConfirm} onOpenChange={setShowLeaveConfirm}>
        <AlertDialogContent className="bg-surface-1 border-border rounded-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display">Leave Room</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to leave "{room.name}"? You can rejoin with the room code later.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="border-border hover:bg-surface-2 rounded-xl">Cancel</AlertDialogCancel>
